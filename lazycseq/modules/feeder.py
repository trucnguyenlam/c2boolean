...
class feeder(core.module.BasicModule):
	...
	def init(self):
		self.addInputParam('backend', 'backend (%sblitz%s, cbmc, esbmc, llbmc, cpachecker, satabs, klee)', 'b', 'cbmc', False)
		#self.addInputParam('show', 'show sequentialized file without analyzing', '', False, True)
		#self.addInputParam('save', 'save sequentialized file as _cs_filename.c', '', False, True)
		self.addInputParam('time', 'analysis time limit (in seconds)', 't', '3600', False)
		self.addInputParam('llvm', 'clang or llvm search path (only for llbmc,klee)', 'p', '', True)
	...
	def loadfromstring(self, string, env):
		...
		out, err, code = command.run(timeout=int(timelimit))   # store stdout, stderr, process' return value
