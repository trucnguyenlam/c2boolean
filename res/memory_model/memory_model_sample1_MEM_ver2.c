...
extern unsigned __CPROVER_bitvector[6] nondet_int();
/*
...
unsigned __CPROVER_bitvector[6] pos;
...
// New variable for keeping things
_Bool TRACK_VALID[TRACK_SIZE];
unsigned __CPROVER_bitvector[6] TRACK_POS[TRACK_SIZE];
...
{
  static unsigned int num_malloc = 3;
...
int READ_POS(unsigned __CPROVER_bitvector[6] position)
{
  if(TRACK_VALID[position])
  {
    return TRACK_VALUE[TRACK_POS[position]];
  }
  return READ_ADDR(TRACK_VALUE[position]);
}
...
void WRITE_POS(unsigned __CPROVER_bitvector[6] position, int value)
{
  if (TRACK_VALID[position])
  {
    TRACK_VALUE[TRACK_POS[position]] = value;
  }
  else
  {
...
int main() {
  ...
  TRACK_VALUE[0] = my_malloc(CONST * 2); // 2 is the size of a node
  TRACK_VALID[0] = 0;
  ...
  TRACK_VALID[2] = TRACK_VALID[0];
  TRACK_POS[2] = TRACK_POS[0];
  ...
  while (__VERIFIER_nondet_int()) {
    ...
    // p->n = t;
    WRITE_NORMAL(TRACK_VALUE[2] + 1, TRACK_VALUE[1]);
    TRACK_VALID[pos] = TRACK_VALID[1];
    TRACK_POS[pos] = TRACK_POS[1];

    // p = p->n;
    TRACK_VALUE[2] = TRACK_VALUE[pos];
    TRACK_VALID[2] = TRACK_VALID[pos];
    TRACK_POS[2] = TRACK_POS[pos];
  }
