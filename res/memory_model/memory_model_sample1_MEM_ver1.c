...
int main() {
  ...
  TRACK_VALUE[0] = my_malloc(CONST * 3); // 3 is the size of a node
  //
  // List t;
  ...
  while (__VERIFIER_nondet_int()) {
    // p->h = 1;
    WRITE_ADDR(TRACK_VALUE[2] + 1, 1);

    // t = (List) malloc(CONST*sizeof(struct node));
    TRACK_VALUE[1] = my_malloc(CONST * 3);

    // p->n = t;
    WRITE_ADDR(TRACK_VALUE[2] + 2, TRACK_VALUE[1]);
    // p = p->n;
    TRACK_VALUE[2] = READ_ADDR(TRACK_VALUE[2] + 2);
  }


  while (__VERIFIER_nondet_int()) {
    // p->h = 2;
    WRITE_ADDR(TRACK_VALUE[2] + 1, 2);
  ...
  // while (p->h == 1)
  while (READ_ADDR(TRACK_VALUE[2] + 1) == 1)
  {
    // p = p->n;
    TRACK_VALUE[2] = READ_ADDR(TRACK_VALUE[2] + 2);
  }
  // while (p->h == 2)
  while (READ_ADDR(TRACK_VALUE[2] + 1) == 2)
  {
    // p = p->n;
    TRACK_VALUE[2] = READ_ADDR(TRACK_VALUE[2] + 2);
  }
  // if (p->h == 3)
  if (READ_ADDR(TRACK_VALUE[2] + 1) == 3)
  {
    assert(0);
  }
